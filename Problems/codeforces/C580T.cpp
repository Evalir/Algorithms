...
void dfs(Node& current, const int nodesWithCats, vector<Node> &nodes) {
    ...
    if (nodesWithCats > M) {
        //cout << "Cannot keep on, node " << current.identifier << endl;
    }
    else {
...
// /// @param nodesWithCats: is the number of consecutive ancestor nodes that has cats.
// /// @param nodes: list of all nodes with the graph info
// void dfs(Node& current, int nodesWithCats, vector<Node> &nodes) {
//     cerr << 'E' << endl;
// 	bool isChild = true;
// 	current.isVisited = true;
//     if (current.hasCats) nodesWithCats++;
//     else nodesWithCats = 0;
//     cout << nodesWithCats << " CATCOUNT ON NODE " << current.identifier << endl;
// 	for (int id : current.adj ) {
// 		Node &v = nodes[id];
// 		if (!v.isVisited) {
// 			isChild = false; // since it called another node it means it is not a child
// 			// do recursion
//             dfs(v, nodesWithCats, nodes);
// 		}
// 	}
// 	if (isChild) {
// 		// i am a child do any logic
//         canGo++;
//         //cout << "I am a child!" << ' ' << current.hasCats << endl;
//         //cout << nodesWithCats << " GOT TO LEAF " << current.identifier <<  " WITH CATS" << endl;
// 	}
// }
